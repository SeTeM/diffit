class CreateDiffits < ActiveRecord::Migration
  def up
    enable_extension "hstore"

    create_table :diffits do |t|
    ...
    add_index :diffits, :last_changed_at
    add_index :diffits, [:table_name, :record_id], unique: true
    ...
    execute %[
      CREATE OR REPLACE RULE "insert_or_update_to_diffits" AS ON INSERT TO diffits
      WHERE EXISTS
        ( SELECT 1
          FROM diffits
          WHERE
  ...
  def down
    execute %[
      DROP RULE insert_or_update_to_diffits ON diffits;
      DROP FUNCTION diffit_changes;
    ]
    remove_index :diffits, :last_changed_at
    remove_index :diffits, [:table_name, :record_id]
    drop_table :diffits
  end
end
